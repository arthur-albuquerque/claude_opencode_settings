...
- Jobs of **3+ independent, non-overlapping tasks** (migrations, test backfills, multi-module features, overnight runs): invoke the `delegate-workflow` skill. It runs the batch through an ultracode Workflow — tier-capped worker concurrency (cheap models fan wide, expensive models stay narrow), the escalation ladder encoded as a retry loop, per-task QA by cheap Claude agents, budget-resumable via `resumeFromRunId` — while you keep decomposition and final review. For 1–2 tasks, delegate directly as above; the workflow tier's wrapper overhead is pure surcharge there.
...
- A direct-delegation wave = ≤3 parallel opencode workers on non-overlapping files — never Claude subagents. (Inside the `delegate-workflow` skill this count cap is replaced by the skill's per-tier concurrency caps — a machine cap plus tighter caps on expensive models, bounding how much unmonitored in-flight work a gateway block can strand.)
