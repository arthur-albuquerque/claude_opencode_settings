...
1. **Deferred tasks (budget block):** report the window + numbers and the exact resume command. Once the window resets, relaunch with `Workflow({scriptPath, resumeFromRunId})` — completed tasks replay from cache; only deferred ones run.
...
| `GoUsageLimitError` / probe exit 2 | Lanes freeze; deferred tasks reported; relaunch after reset with `resumeFromRunId` |
